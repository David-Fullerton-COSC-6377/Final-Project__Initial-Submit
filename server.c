...
Waiting_For_Instructions:
	// say in waiting situation	
		printf("*** {Server} -- Waiting for Instructions ... ***\n");
		
	// set timer back to zero
		TIMING_diff = -1;
		
	// reset locals
		Current_Second_Of_Operation = 0;
	...
	// loop and wait for instructions from client
		...
		{	
			...
			// see if saying to QUIT everything
				if (strcmp(client_message, "~QUIT~") == 0)
				{
				
					// indicate quick close
						printf("*** SERVER -- Quick Stopping without Summary ... ***\n");
			...
			// see if saying to start
				...
				{
					Receive_Message_With_Confirmation__From_Client(client_message, "@");
					//printf(" --> How many seconds {%s} ...\n", client_message);   fuck
					
					How_Many_Seconds_To_Run = char_to_int(client_message);
	...
	// show the count of characters per second
		...
		for (i = 2; i <= (How_Many_Seconds_To_Run + 1)  ; i++)
		{
			printf 
				("   %3i         %8i         %10i \n", 
					i - 1, 
					Metric__Packets_Per_Second[ i ], 
					Metric__Characters_Per_Second[ i ] - 1
				);
	...
	// return and wait for instructions
		goto Waiting_For_Instructions;
