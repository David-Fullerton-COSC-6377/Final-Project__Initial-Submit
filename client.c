...
// variables that can change [DEFAULTS BELOW]
	int		How_Many_Seconds_To_Run = 30;		// [DEFAULT] how many seconds to run
	int		Characters_Per_Packet = 50;			// [DEFAULT] how many characters in a packet
	int	    Max_Chars_Per_Second = 5000;		// [DEFAULT] max characters in a second
...
//
// convert integer into character array
//
//   --> called like this:   int_to_char(100, str);
//
void int_to_char(int iValue, char str[])
{
	//char str[10];
	int i = iValue;
...
//
// Delete a file -- TRUE or FALSE answer
//
int  Files__Delete(char FileName[])
{
	// variables
		int		Result;
...
//
// Does file exist -- TRUE or FALSE answer
//
int  Files__Existence(char FileName[])
{
...
//
void Files__Writing(char FileName[], char WhatToWrite[])
{
	...
	// doing the writing
		fptr = (fopen(FileName, "w"));
			...
			if (fptr == NULL)
			{
				printf("*** WRITING FILE {%s} -- File did not open ... ***\n", FileName);
				exit(1);
			}
...
//
void Files__Reading(char FileName[], char What_Was_Read[])
{

	// write to a file
		FILE	*fptr;
		char	Reading_From_File[100];
	...
	// doing the reading
		fptr = (fopen(FileName, "r"));
			...
			if (fptr == NULL)
			{
				printf("*** READING FILE {%s} -- File did not open ... ***\n", FileName);
				exit(1);
			}
			
		fgets(Reading_From_File, 100, fptr);
		fclose(fptr);	
		
		// printf("READ THIS --> %s\n", Reading_From_File);
		
		strcpy(What_Was_Read, Reading_From_File);
...
{
	// variables
		char	Instructions[MAX_STRING];
		
		char	Server_Reply[MAX_STRING];
		char	TestChar;
		int		Current_Second_Of_Operation = 0;
		
		int		Count_Of_Characters_In_This_Second = 0;
	...
	// how many seconds to count
		char str__How_Many_Seconds_To_Run[MAX_STRING];
		int_to_char(How_Many_Seconds_To_Run, str__How_Many_Seconds_To_Run);
		Send_Message_With_Confirmation__To_Server(str__How_Many_Seconds_To_Run, Server_Reply);  
		
		
	// setup defaults
		Count_Of_Characters_In_This_Second = 0;
			...
			*/
			...
			// send multiple characters
				...
				for (i = 0; i < Characters_Per_Packet; i++)
				{
					...
					// only second how many I want to send per second
						if (Count_Of_Characters_In_This_Second <= Max_Chars_Per_Second)
							{
								message[Over] = Get_Next_Char_From_Test_String();
								Over++;
							
								Count_Of_Characters_In_This_Second ++;
							}						
			...
			// check time
				...
				{				
					// reset number of characters so far in second
						Count_Of_Characters_In_This_Second = 0;
						...
						if (Current_Second_Of_Operation != 0)
							{
								...
								// do not show if over the number -- bez have to run an extra second
					...
					// see if ready to quit
						if ( Current_Second_Of_Operation > (How_Many_Seconds_To_Run + 1))
							{
								break;
							}		
							
					// see if we increase or decrease the speed
						// strcpy(Filename, "nextstep.inf");
						strcpy(Filename, "n");
						if (Files__Existence(Filename) == true)
						{
							// read what to do
								Files__Reading(Filename, Instructions);
								//printf("\nInstructions Received {%s} \n\n", Instructions);
								
							// decide if faster or slower
								if (Instructions[0] == '+')
									{
										printf("\n!!! {INSTRUCTION-FILE} : GOOD --- increasing speed of packets ***\n\n");
										
										Max_Chars_Per_Second = Max_Chars_Per_Second + 1000;
									}
								else if (Instructions[0] == '-')
									{
										printf("\n!!! {INSTRUCTION-FILE} : GOOD --- decreasing speed of packets ***\n\n");
										
										Max_Chars_Per_Second = Max_Chars_Per_Second - 1000;									
									}
								else
									{
										printf("!!! {INSTRUCTION-FILE} : BAD  --- Instruction File is not correct so not changing operation!!! ***\n");
									}
						
							// delete the file
								Files__Delete(Filename);
						}	
				}
...
Change_Configuration_Values:	
	...
	// changing max characters per second
		printf (" Change max of how many characters per second from %i to ?  ", Max_Chars_Per_Second  );
		scanf("%i", &Max_Chars_Per_Second);
		printf ("\n");
...
Show_Configuration_Values:
	...
	printf ("------------------------------------------\n");
	printf ("1) How many seconds to run        = %5i \n", How_Many_Seconds_To_Run  );
	printf ("2) How many characters per packet = %5i \n", Characters_Per_Packet );
	printf ("3) Max characters per second      = %5i \n", Max_Chars_Per_Second ); 
	...
	Process_Menu_Option:
				switch (Menu_Option)
				{
					...
					case '4':   // immediate quit
						...
						// tell server to end				
							Send_Message_With_Confirmation__To_Server("~QUIT~", Server_Reply);
							
						// indicate quick close
							printf("*** CLIENT -- Quick Stopping without Summary ... ***\n");
